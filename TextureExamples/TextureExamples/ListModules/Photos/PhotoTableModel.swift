...
extension PhotoTableModel {
    ...
    func attributedStringForDescription(withSize size: CGFloat) -> NSAttributedString {
        ...
        return NSAttributedString(string: descriptionField ?? "", attributes: attributes)
    }

    func attributedStringLikes(withSize size: CGFloat) -> NSAttributedString {
        guard let formattedLikesNumber = NumberFormatter.decimalNumberFormatter.string(from: NSNumber(value: likes)) else {
    ...
    func attributedStringForTimeSinceString(withSize size: CGFloat) -> NSAttributedString {
        guard let date = Date.iso8601Formatter.date(from: self.createdAt ?? "") else {
            return NSAttributedString();
        }
